...
该命令会重新读取邮件正文，严格区分固定时间笔试、非固定时间笔试、“约面”、AI 面试和已确认人工面试。固定笔试和人工面试在缺少结束时间时默认占用 1 小时；非固定笔试以截止时间为日程开始、占用 30 分钟。所有日程提前 1 天和 30 分钟提醒，只写入用户主日历，不额外创建飞书视频会议。日程队列和幂等键保存在本地 SQLite，临时失败会在后续轮询重试且不会重复调用模型。旧命令 `./tracker repair-interviews` 仍可使用。
