...
- 面试日程：只有邮件已经确认唯一人工面试时间时，才在飞书主日历创建日程；AI 面试、笔试和仍需自行选时间的“约面”不会建日程。
...
启用日历后，修正历史星标面试邮件并补建仍在未来的已确认人工面试日程：

```bash
./tracker repair-interviews
```

该命令会重新读取邮件正文，严格区分“约面”、AI 面试和已确认人工面试。日程默认占用 1 小时（邮件提供结束时间时以邮件为准），提前 1 天和 30 分钟提醒；只写入用户主日历，不额外创建飞书视频会议。日程队列和幂等键保存在本地 SQLite，临时失败会在后续轮询重试且不会重复调用模型。
...
测试覆盖结构化输出校验、广告过滤、投递失败处理、截止时间约束、约面/AI 面试纠错、日程创建门禁、流程推进策略、岗位归一化、IMAP 星标、Coremail 原生待办和飞书日历队列。
