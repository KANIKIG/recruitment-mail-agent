...
- 结构化流程：支持 `待确认`、`投递`、`测评&AI面`、`笔试`、`约面`、`技术面`、`HR面`、`主管面`、`Offer`、`已挂`。
...
- 笔试与面试日程：固定时间笔试按开考时间创建，非固定时间笔试按最晚完成时间创建“笔试截止”日程；人工面试仅在唯一时间已确认后创建。AI 面试和仍需自行选时间的“约面”不会建日程。
- 日历可靠性：提醒直接由邮件结构化结果入队，不依赖飞书表格行能否匹配；同一邮件重复分类或后续提醒邮件使用同一会议入口时不会重复建日程，表格匹配失败会输出显式告警。
- 约面纠错：自助预约、选择可选场次或进入系统选时间统一标记为“约面”；预约链接不会被当作会议链接。
...
IMAP（BODY.PEEK）
  ...
  → 笔试有固定时间/截止时间或人工面试已确认时创建飞书日历日程
```

飞书表包含七列：`公司名称`、`岗位名称`、`流程状态`、`企业类型`、`截止时间`、`更新时间`、`投递时间`。企业类型为民营企业、央国企、事业单位或外企；默认表格与看板按更新时间倒序排列，不同状态使用不同标签颜色。
...
LARK_BASE_TOKEN=
LARK_TABLE_ID=
LARK_CLI=./node_modules/.bin/lark-cli
...
npm run lark -- auth login --recommend --domain calendar
...
恢复默认列顺序可执行 `./tracker arrange-fields`。
...
该命令只检查 `SYNC_SINCE_DATE` 之后的星标邮件，优先复用已有模型结果；仅未缓存邮件会调用模型。对于模型漏掉的明确“若干小时/天内”或“链接有效期”期限，会按收信时间确定性补算，并同步补齐飞书中仍为空的截止时间。它不会改变增量 UID 游标，重复执行也不会重复创建同日期待办。

启用日历后，修正历史星标笔试/面试邮件并补建仍在未来的日程：

```bash
./tracker repair-calendar
```

该命令会重新读取邮件正文，严格区分固定时间笔试、非固定时间笔试、“约面”、AI 面试和已确认人工面试。固定笔试和人工面试在缺少结束时间时默认占用 1 小时；非固定笔试以截止时间为日程开始、占用 30 分钟。所有日程提前 1 天和 30 分钟提醒，只写入用户主日历，不额外创建飞书视频会议。日程队列和幂等键保存在本地 SQLite，临时失败会在后续轮询重试且不会重复调用模型。即使公司或岗位名称暂时无法匹配表格已有行，未来日程仍会创建并留下本地告警。旧命令 `./tracker repair-interviews` 仍可使用。
...
测试覆盖结构化输出校验、广告过滤、投递失败处理、固定/非固定笔试时间、截止时间约束、约面/AI 面试纠错、日程创建门禁、流程推进策略、岗位归一化、IMAP 星标、Coremail 原生待办和飞书日历队列。
